{
  "helpfixer" = {
    "src" = "https://minecraft.curseforge.com/projects/helpfixer/files/2223813/download";
    "version" = "1.0.7";
    "title" = "HelpFixer";
    "projectID" = 223797;
    "filename" = "HelpFixer-1.0.7.jar";
    "dependencies" = [];
    "encoded" = "HelpFixer-1.0.7.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/helpfixer";
    "type" = "remote";
    "md5" = "0d6ce4776f981f97a13ab02029b5fb4d";
  };
  "inventory-tweaks" = {
    "src" = "https://minecraft.curseforge.com/projects/inventory-tweaks/files/2210792/download";
    "version" = "1.59-dev-152";
    "title" = "Inventory Tweaks";
    "projectID" = 223094;
    "required" = false;
    "filename" = "InventoryTweaks-1.59-dev-152.jar";
    "dependencies" = [];
    "encoded" = "InventoryTweaks-1.59-dev-152.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/inventory-tweaks";
    "type" = "remote";
    "side" = "client";
    "isDefault" = true;
    "md5" = "83f7ddcd5f5fd8d640aa15dc62acc6bf";
  };
  "journeymap-32274" = {
    "src" = "https://minecraft.curseforge.com/projects/journeymap-32274/files/2367915/download";
    "version" = "1.7.10-5.1.4p2-unlimited";
    "title" = "JourneyMap";
    "projectID" = 32274;
    "required" = false;
    "filename" = "journeymap-1.7.10-5.1.4p2-unlimited.jar";
    "dependencies" = [];
    "encoded" = "journeymap-1.7.10-5.1.4p2-unlimited.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/journeymap-32274";
    "type" = "remote";
    "side" = "client";
    "isDefault" = true;
    "md5" = "0c3daa19789a67da328437a8635c2cf3";
  };
  "loading-profiler" = {
    "src" = "https://minecraft.curseforge.com/projects/loading-profiler/files/2257233/download";
    "version" = "1.7.10-1.2";
    "title" = "Loading Profiler";
    "projectID" = 228513;
    "filename" = "LoadingProfiler-MC1.7.10-1.2.jar";
    "dependencies" = [];
    "encoded" = "LoadingProfiler-MC1.7.10-1.2.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/loading-profiler";
    "type" = "remote";
    "md5" = "4514685906845da7db6ee6005ca917da";
  };
  "lunatriuscore" = {
    "src" = "https://minecraft.curseforge.com/projects/lunatriuscore/files/2225146/download";
    "version" = "1.7.10-1.1.2.21-universal";
    "title" = "LunatriusCore";
    "projectID" = 225605;
    "filename" = "LunatriusCore-1.7.10-1.1.2.21-universal.jar";
    "dependencies" = [];
    "encoded" = "LunatriusCore-1.7.10-1.1.2.21-universal.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/lunatriuscore";
    "type" = "remote";
    "md5" = "435b2890f90cc256e0045eb559512eaf";
  };
  "reauth" = {
    "src" = "https://minecraft.curseforge.com/projects/reauth/files/2323884/download";
    "version" = "3.2";
    "title" = "ReAuth";
    "projectID" = 237701;
    "filename" = "ReAuth-3.2.jar";
    "dependencies" = [];
    "encoded" = "ReAuth-3.2.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/reauth";
    "type" = "remote";
    "side" = "client";
    "md5" = "c01189492a3881212a2e523c5ff7182b";
  };
  "schematica" = {
    "src" = "https://minecraft.curseforge.com/projects/schematica/files/2274295/download";
    "version" = "1.7.10-1.7.6.131-universal";
    "title" = "Schematica";
    "projectID" = 225603;
    "filename" = "Schematica-1.7.10-1.7.6.131-universal.jar";
    "dependencies" = [225605];
    "encoded" = "Schematica-1.7.10-1.7.6.131-universal.jar";
    "projectPage" = "https://minecraft.curseforge.com/projects/schematica";
    "type" = "remote";
    "side" = "client";
    "md5" = "6318edf4fc3f3f993d41c385cd87e8ef";
  };
}
