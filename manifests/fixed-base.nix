[
  {
    name = "RTG-1.7.10-1.1.1";
    filename = "RTG-1.7.10-1.1.1.jar";
    url = "https://minecraft.curseforge.com/projects/realistic-terrain-generation/files/2319524/download";
    md5 = "88b73ebbf92f06a2988a65bc0912dba9";
  }
  {
    name = "ClimateControl-0.6.beta59.jar";
    filename = "ClimateControl-0.6.beta59.jar";
    url = "https://minecraft.curseforge.com/projects/climate-control/files/2304112/download";
    md5 = "d598060b08571f92959706f2eeaea2bc";
  }
  {
    name = "ATG_0.12.0_for_Minecraft_1.7.10";
    filename = "ATG 0.12.0 for Minecraft 1.7.10.jar";
    url = "https://minecraft.curseforge.com/projects/alternate-terrain-generation/files/2231153/download";
    md5 = "624d457569301c4aef58eeeca64f0b6f";
  }
  {
    name = "Streams-0.2.jar";
    filename = "Streams-0.2.jar";
    url = "https://minecraft.curseforge.com/projects/streams/files/2281587/download";
    md5 = "4aefeee9e2c22a11b6f6c8b76ab7a267";
  }
  {
    name = "Farseek-1.0.11.jar";
    filename = "Farseek-1.0.11.jar";
    url = "https://minecraft.curseforge.com/projects/farseek/files/2281555/download";
    md5 = "e6deb2987bc0c8feb3de9edd87d89745";
  }
  {
    name = "Highlands-1.7.2-v-2.2.3.jar";
    filename = "Highlands-1.7.2-v-2.2.3.jar";
    url = "https://minecraft.curseforge.com/projects/highlands/files/2227924/download";
    md5 = "0a6dfa4d6bf9e5b6a3287a694ce0bb57";
  }
  {
    name = "roguelike-1.7.10-1.5.0b.jar";
    filename = "roguelike-1.7.10-1.5.0b.jar";
    url = "https://minecraft.curseforge.com/projects/roguelike-dungeons/files/2294894/download";
    md5 = "d62d30d52d3f51dc39bdc61ce224e6c6";
  }
  {
    name = "appliedenergistics2-rv2-stable-10.jar";
    filename = "appliedenergistics2-rv2-stable-10.jar";
    url = "https://minecraft.curseforge.com/projects/applied-energistics-2/files/2245762/download";
    md5 = "f7a60fa036240da11da4a739c5c4137d";
  }
]
